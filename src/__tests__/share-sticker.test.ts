...
describe('generateStickerPng — Canvas 2D', () => {
  ...
  it('throws when toBlob returns null', async () => {
    const mockGradient2 = { addColorStop: vi.fn() }
    const ctx2d = {
      clearRect:            vi.fn(),
      fillRect:             vi.fn(),
      beginPath:            vi.fn(),
      ...
      restore:              vi.fn(),
      measureText:          vi.fn().mockReturnValue({ width: 200 }),
      createRadialGradient: vi.fn().mockReturnValue(mockGradient2),
      createLinearGradient: vi.fn().mockReturnValue(mockGradient2),
      font: '', fillStyle: '', strokeStyle: '', lineWidth: 0, lineCap: '', lineJoin: '',
