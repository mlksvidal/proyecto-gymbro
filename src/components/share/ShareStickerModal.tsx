// ============================================================
// GYMBRO — ShareStickerModal (Sprint 15)
...
import { StickerCanvas, type StickerData } from './StickerCanvas'
...
export function ShareStickerModal({ open, onClose, data }: ShareStickerModalProps) {
  ...
  // Revoke previous object URL when previewUrl changes
  useEffect(() => {
  ...
  // ── generate function (plain async, no useCallback) ──────────
  // Declared before the effect that calls it — avoids
  // "accessed before declaration" lint error.
  async function generate() {
    if (!canvasRef.current) return
    setStatus('generating')
    try {
      const blob = await generateStickerPng(canvasRef.current)
  ...
  // Auto-generate when modal opens; reset state first via initializer pattern
  ...
  useEffect(() => {
    if (open && !openRef.current) {
      ...
      setPreviewUrl(null)
      setToastMsg(null)
      // Defer state reads to next tick so setPreviewUrl(null) settles
  ...
  return (
    <>
      ...
      <StickerCanvas ref={canvasRef} {...data} />
              ...
              {status === 'done' && previewUrl && (
                <motion.div
                  ...
                  style={{
                    ...
                    flexShrink: 0,
                    // Checkerboard pattern to visually confirm transparency
                    backgroundImage:
